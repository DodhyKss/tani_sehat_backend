...
@section('content')
<div class="mb-8">
    <h1 class="text-3xl md:text-4xl font-extrabold text-black mb-2">Dashboard Kader</h1>
    <p class="text-primary-800 text-lg font-bold">Monitoring kesehatan warga dampingan Anda</p>
</div>
...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-8 mb-10">
    <div class="bg-gradient-to-br from-primary-600 to-primary-800 rounded-[2rem] p-6 md:p-8 text-white shadow-xl shadow-primary-900/10 border border-primary-500/20">
        ...
        <h3 class="text-primary-100 text-sm font-bold uppercase tracking-wider mb-2">Warga Saya</h3>
    ...
    <div class="bg-gradient-to-br from-purple-600 to-purple-800 rounded-[2rem] p-6 md:p-8 text-white shadow-xl shadow-purple-900/10 border border-purple-500/20">
        <div class="flex items-center justify-between mb-6">
            <div class="p-3 bg-white/20 rounded-2xl">
                <svg class="w-8 h-8" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>
            </div>
            <span class="text-xs font-black uppercase tracking-widest opacity-60">Status</span>
        </div>
        <h3 class="text-purple-100 text-sm font-bold uppercase tracking-wider mb-2">Chat Baru</h3>
        <div class="text-3xl md:text-5xl font-black" id="valChat">-</div>
    </div>
</div>
    ...
    <div id="capture-td-status" class="bg-white rounded-2xl md:rounded-[2.5rem] shadow-sm border border-gray-100 p-5 md:p-10 hover:shadow-2xl hover:shadow-primary-100/10 transition-all duration-500 overflow-hidden">
        <div class="flex items-center justify-between mb-6 md:mb-8">
            <div>
                ...
                <p class="text-[9px] md:text-[10px] font-black text-primary-400 uppercase tracking-[0.2em] mt-1">Distribusi Warga Dampingan</p>
            </div>
    ...
    <div id="capture-gad-status" class="bg-white rounded-2xl md:rounded-[2.5rem] shadow-sm border border-gray-100 p-5 md:p-10 hover:shadow-2xl hover:shadow-indigo-100/10 transition-all duration-500 overflow-hidden">
        <div class="flex items-center justify-between mb-6 md:mb-8">
            <div>
                ...
                <p class="text-[9px] md:text-[10px] font-black text-primary-400 uppercase tracking-[0.2em] mt-1">Kesehatan Psikologis Warga</p>
            </div>
            <div class="flex items-center gap-3">
                <div class="hidden sm:flex gap-2 mr-2">
                    <span class="w-2 h-2 rounded-full bg-emerald-500"></span>
...
<div class="bg-white rounded-[2.5rem] shadow-xl shadow-primary-900/5 border border-primary-100 p-6 md:p-10 mb-10 overflow-hidden">
    <div class="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-4">
        <div>
            ...
            <p class="text-primary-800 text-base font-bold mt-1 uppercase tracking-widest opacity-60">Data Warga Dampingan Anda</p>
        </div>
        <div class="flex flex-wrap items-center gap-3">
            ...
            <a href="/kader/kesehatan" class="bg-primary-50 text-primary-800 hover:bg-primary-100 px-6 py-3 rounded-2xl font-black text-sm flex items-center gap-2 transition-all group uppercase tracking-widest">
                LIHAT RIWAYAT
                ...
                tbody.innerHTML = res.data.map(item => `
                    <tr class="hover:bg-primary-50/50 transition-colors group">
                        ...
                        <td class="px-6 py-6 text-center border-x border-primary-50">
                            ...
                            <span class="px-3 py-1 text-[10px] rounded-lg uppercase font-black tracking-widest ${getBadge(item.td.status_awal)}">${item.td.status_awal.replace('pre_hipertensi', 'Pra-Hipertensi').replace('_', ' ')}</span>
                        </td>
                        <td class="px-6 py-6 text-center border-r border-primary-50">
                            ...
                            <span class="px-3 py-1 text-[10px] rounded-lg uppercase font-black tracking-widest ${getBadge(item.td.status_akhir)}">${item.td.status_akhir.replace('pre_hipertensi', 'Pra-Hipertensi').replace('_', ' ')}</span>
                        </td>
                        <td class="px-6 py-6 text-center border-r border-primary-50">
                ...
                cards.innerHTML = res.data.map(item => `
                    <div class="bg-white rounded-[2rem] p-6 shadow-xl shadow-primary-900/5 border border-primary-100 mb-4">
                        ...
                        <div class="mt-6 pt-4 border-t border-primary-50 text-center">
                             <span class="px-4 py-2 rounded-2xl text-[10px] font-black uppercase tracking-widest ${item.status_perubahan === 'Ada Perubahan' ? 'bg-amber-100 text-amber-800 border-2 border-amber-200' : (item.status_perubahan === 'Tetap' ? 'bg-emerald-100 text-emerald-800' : 'bg-primary-50 text-primary-300')}">
