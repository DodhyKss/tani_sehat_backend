...
<div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-6">
    <div>
        <h1 class="text-3xl md:text-4xl font-extrabold text-black mb-2 tracking-tight">Rekomendasi & Edukasi</h1>
        <p class="text-primary-800 text-lg font-bold uppercase tracking-widest opacity-60">Materi Pembelajaran Berkualitas Untuk Warga Binaan</p>
    </div>
</div>
...
function renderItems(containerId, items, color, detailFn) {
    ...
    if (items.length === 0) {
        container.innerHTML = `<p class="text-center py-6 text-${color}-100 font-black uppercase tracking-widest text-xs italic opacity-60">Tidak ada data</p>`;
        return;
    }
...
function renderOlahragas(olahragas) {
    ...
    if (olahragas.length === 0) {
        container.innerHTML = '<p class="text-center py-6 text-orange-100 font-black uppercase tracking-widest text-xs italic opacity-60">Tidak ada program</p>';
        return;
    }
